    ...
    begin
      ...
      begin
        ...
        if not LAitem.InsertToDB(SAitem.VList.ValueFromIndex[i]) then
          SystemLog(Format(' %s,插入数据：%s失败',[tablename,SAitem.VList.ValueFromIndex[i]]));
      end
      else                                                         //本地更新记录
      begin
        // 若有查看数据是否一致
        ...
        begin
          ...
          if not LAitem.UpdateToDB(SAitem.VList.ValueFromIndex[i]) then
            SystemLog(Format(' %s,更新数据：%s失败',[tablename,SAitem.VList.ValueFromIndex[i]]));
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        if not LAitem.DeleteToDB(LAitem.VList.ValueFromIndex[i]) then
          SystemLog(Format(' %s,删除数据：%s失败',[tablename,LAitem.VList.ValueFromIndex[i]]));
      end;
    end;
    result := true;
  finally
