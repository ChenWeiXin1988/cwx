...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      SystemLog(Format('数据连接失败 - [Failure:%s]', [E.Message]));
    end;
  end;
end;
...
begin
  ...
  try
    try
      ...
      SystemLog(Format('%s-Update：[%s]', [sTableName,FDQuery.SQL.Text]));
      FDQuery.ExecSQL;
    except
      on E: Exception do
      begin
        SystemLog(Format('数据%s更新失败 - [Failure:%s]', [sTableName,E.Message]));
      end;
    end;
    Result := True;
  finally
