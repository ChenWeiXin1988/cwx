...
var
  cFile: string;
begin
  cFile := Format('Log\DM_Log_%s.txt', [FormatDateTime('YYYYMMDD', Now)]);
...
begin
  ...
  try
    try
      ...
      SystemLog(' 操作注册表开机启动 - [Success]');
    except
      on E: Exception do
      begin
        SystemLog(Format('操作注册表开机启动 - [Failure:%s]', [E.Message]));
      end;
    end;
  finally
    regf.Free;
