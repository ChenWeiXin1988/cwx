...

{
 ...
 多项式   : x16+x12+x5+1
 简记式   : 1021
 标准引用 : ISO HDLC, ITU X.25, V.34/V.41/V.42, PPP-FCS
}
...
uses
  ...
  function CRC(a_strPuchMsg:TBytes):LongWord;
...
function CRC(a_strPuchMsg:TBytes):LongWord; //计算CRC
var
  l_CRCH, l_CRCL, l_Index, l_DataLen, i:LongWord;
begin
  l_DataLen:= High(a_strPuchMsg);
  l_CRCH := $FF;
  l_CRCL := $FF;
  l_Index :=0;
  i:=0;
  ...
  begin
    l_Index := l_CRCH xor Integer(a_strPuchMsg[i]);
    l_CRCH := l_CRCL xor CRCH[l_Index];
    l_CRCL := CRCL[l_Index];
    Inc(i);
  end;
  Result:= (l_CRCH shl 8) or l_CRCL;
end;
