...
begin
  ...
  ReRead:
    ...
    begin
      ...
      for i := 0 to Len - 1 do
      begin
        if Arr[i]= GDataReceive_Header then         //包头
        begin
          iRight := Len - iErr;                     //iRight 从0XEE开始的数据包长度
          if i > 0 then                             //第一个字符就是则不用剔除
          begin
            SetLength(Arr, iErr);
            Len:=Pop(Arr, iErr);                       //直接把0xEE包头之前的数据全部读取,将圆形缓冲区内数据清空，0xEE及0xEE之后的保留 ，如果读取未成功，则下一次进行
            if (Len = 0) or (Len <> iErr) then Exit;  //读取并清空数据未成功，直接退出
            goto ReRead;
          end;
          if iRight < GHeaderLen then               //一个数据包至少有 包头EE 数据长度 XX 和校验2个字节；所以，如果小于3就表明数据还未收完整
          begin
            Exit;
          end
          else                   //大于3个时候，需要判断包是否完整，包括校验
          begin
            iLen := Arr[i+1]+ 4 ;  //整个数据包的长度 = 据长度 + GHeaderLen字节
